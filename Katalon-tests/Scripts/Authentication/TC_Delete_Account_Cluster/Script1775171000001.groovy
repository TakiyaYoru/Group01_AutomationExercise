...
assert isLoggedIn() : '[TC_Delete_Account_Cluster] Cannot establish logged-in state'

WebUI.click(findTestObject('Authentication/btn_Delete_Account'))
boolean deleted = WebUI.verifyElementPresent(findTestObject('Authentication/lbl_Account_Deleted'), 5, FailureHandling.OPTIONAL)
if (!deleted) {
	deleted = WebUI.verifyTextPresent('Account Deleted', false, FailureHandling.OPTIONAL)
}
if (!deleted) {
	deleted = WebUI.getUrl().contains('delete_account')
}
assert deleted : '[TC_Delete_Account_Cluster] Expected account deleted state but not found'
