...
if (exp == 'success') {
	...
	assert hasSuccess : '[TC_Checkout_Payment] Expected success but order success was not found'
} else {
	WebUI.verifyElementNotPresent(findTestObject('Checkout/lbl_Order_Success'), 5)
	WebUI.verifyElementPresent(findTestObject('Checkout/btn_Pay_Confirm'), 5)
}
